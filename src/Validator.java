...
//		}
	...
	private boolean IsSelectValid(String inp) {
		return inp
				.toLowerCase()
				.matches(
						"^(select)(\\ *\\*\\ *)\\ +from\\ +[_a-z][a-z0-9_]*\\ *(where\\ +[_a-z][a-z0-9_]*\\ *((>)|(<)|(=)|(>=)|(<=)|(==))\\ *(('.*')|([0-9]+))\\ *)?(\\ *order\\ +by(\\ *(('?.*'?)|([0-9]+))\\ *,?)*(desc|asc)\\ *)?;?")
				|| inp.toLowerCase()
						.matches(
								"^(select)\\ +(distinct)?\\ *(\\w+,?)+\\ *from\\ +\\w+\\ *(where\\ +\\w+\\ *((>)|(<)|(=)|(>=)|(<=)|(==))\\ *(\\w+)\\ *)?(\\ *order\\ +by(\\ *(('?.*'?)|([0-9]+))\\ *,?)*\\ *(desc|asc)\\ *)?;?");
	}
	...
	private boolean IsInsertValid(String inp) {
		return inp
				.toLowerCase()
				.matches(
						"^(insert)\\ +(into)\\ +[_a-z][a-z0-9_]*\\ *\\(\\ *(\\ *\\w+\\ *,?)+\\ *\\)\\ *values\\ *\\(\\ *(\\ *'?\\{?('?\\w+'?,?)+\\}?'?,?)+\\)\\ *;?");
	...
	private boolean IsDeleteValid(String inp) {
		return inp
				.toLowerCase()
				.matches(
						"^(delete)((\\ *\\*\\ *)) +(from)\\ +[_a-z][a-z0-9_]*\\ *(where\\ +[_a-z][a-z0-9_]*\\ *=\\ *(('.*')|([0-9]+))\\ *)?;?")
				|| inp.toLowerCase()
						.matches(
								"^(delete)\\ +(from)\\ +[_a-z][a-z0-9_]*\\ +where\\ +[_a-z][a-z0-9_]*\\ *=\\ *(('.*')|([0-9]+))\\ *;?");
	}
	...
	private boolean IsCreateValid(String inp) {
		if (inp.toLowerCase().matches(
				"^(create)\\ +(database)\\ +[_a-z][a-z0-9_]*\\ *;?"))
			return true;
		return inp
				.toLowerCase()
				.matches(
						"^(create)\\ +(table)\\ +[_a-z][a-z0-9_]*\\ *\\(\\ *(\\ *\\w+\\ +((varchar\\([0-9]+\\))|(integer)|(tinyint)|(smallint)|(int)|real|float|double|bigint|long|(integer array\\[[0-9]+\\])|(varchar array\\[[0-9]+\\])|(char\\([0-9]+\\))|(longvarchar\\([0-9]+\\)))\\ *,?)*\\)\\ *;?");
	}
}